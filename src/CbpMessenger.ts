...
export class CbpMessenger {
  ...
  public async sendTextMessage(to:string,msg:string):Promise<boolean>
  {
    return this.sendAsync<jbdt.IMessengerData>(
      ...
      { operation: "sendMessage",messengerName:this.messengerName,data:{to,msg} }
    );
  }

  public async sendFiles(to:string,files:Array<string>):Promise<Array<boolean>>
  {
    return this.sendAsync<jbdt.IMessengerData>(
      jbdt.SDKClientActions.MESSENGER,
      { operation: "sendFiles",messengerName:this.messengerName,data:{to,files} }
    );
  }

  public async getContacts(): Promise<
    Array<jbdt.IMessengerContact>
  > {
    return this.sendAsync<jbdt.IMessengerData>(
      jbdt.SDKClientActions.MESSENGER,
